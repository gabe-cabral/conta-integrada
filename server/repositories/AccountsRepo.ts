...
class AccountsRepo extends BaseSecureUserScopedRepo<BankAccount, AccountDbDocument> {
  ...
  override async mapUpdateDocument(record: UpdateUserScopedRecord<BankAccount>): Promise<Partial<AccountDbDocument>> {
    ...
    if (record.number !== undefined) data.number = record.number ? await this.encryptRandom(record.number) : null;
